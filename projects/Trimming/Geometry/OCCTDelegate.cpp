...
OCCT_DELEGATE_CURVE::OCCT_DELEGATE_CURVE(NurbsCurve *nbs) {
  ...
  for (Standard_Integer i = 0; i < nbs->m_spans.size(); i++) {
    if ((i < nbs->m_spans.size() - 1) &&
        (nbs->m_spans[i] == nbs->m_spans[i + 1])) {
      flag = true;
    }
  ...
  m_this =
      new Geom2d_BSplineCurve(points, weights, knots, mults,
...
void OCCT_DELEGATE_CURVE::get_intesectsWithLine(const Point &p1,
  ...
  Geom2dAPI_InterCurveCurve intersector(m_this, line,
                                        FLOAT_ZERO_GEOMETRY_COMPUTE);
...
bool OCCT_DELEGATE_CURVE::get_pointProjecionOnCurve(const Point &interp,
                                                    ...
                                                    const double s,
                                                    const double t) {
  Geom2dAPI_ProjectPointOnCurve projector(gp_Pnt2d(interp[0], interp[1]),
                                          m_this);
  // if (projector.NbPoints() > 0 && projector.LowerDistance() <
  // FLOAT_ZERO_GEOMETRY)
...
vector<double> OCCT_DELEGATE_CURVE::get_extrem(Frame &conserve_frame) {
  ...
  Geom2dAPI_ExtremaCurveCurve extrem_v(m_this, line, m_this->FirstParameter(),
                                       m_this->LastParameter(), 0,
                                       conserve_frame.get_size(0));
  for (auto i = 1; i <= extrem_v.NbExtrema(); i++) {
    gp_Pnt2d p1, p2;
    extrem_v.Points(i, p1, p2);
    double para = 0;
    if (get_pointProjecionOnCurve(Point{p1.X(), p1.Y()}, para,
                                  m_this->FirstParameter(),
                                  m_this->LastParameter())) {
  ...
  Geom2dAPI_ExtremaCurveCurve extrem_h(m_this, line, m_this->FirstParameter(),
                                       m_this->LastParameter(), 0,
                                       conserve_frame.get_size(1));
...
OCCT_NURBS_SURFACE::OCCT_NURBS_SURFACE(NurbsFace *nbs) {
  TColgp_Array2OfPnt poles(1, nbs->m_controlPoints[0].size(), 1,
                           nbs->m_controlPoints.size());
  TColStd_Array2OfReal weight(1, nbs->m_controlPoints[0].size(), 1,
                              nbs->m_controlPoints.size());

  TColStd_Array1OfReal uknot(1, nbs->m_spans[0].size()),
      vknot(1, nbs->m_spans[1].size());
  TColStd_Array1OfInteger umult(1, nbs->m_spans[0].size()),
      vmult(1, nbs->m_spans[1].size());

  for (size_t j = 1; j <= nbs->m_controlPoints.size(); j++) {
    for (size_t i = 1; i <= nbs->m_controlPoints[0].size(); i++) {
      Point3D &temp =
          *((*(nbs->m_controlPoints.begin() + j - 1)).begin() + i - 1);
  ...
  m_this = new Geom_BSplineSurface(poles, weight, uknot, vknot, umult, vmult,
                                   nbs->m_order[0] - 1, nbs->m_order[1] - 1);
}
...
std::vector<double> OCCT_NURBS_SURFACE::get_1stFundForm(int i, int j) {
  ...
  center *= (10.0 / static_cast<double>((m_this->UDegree() + 1) *
                                        (m_this->VDegree() + 1)));
  ...
  for (auto i = ulow; i < uhigh; i++) {
    for (auto j = vlow; j < vhigh; j++) {
      pu.emplace_back(m_this->Weight(i, j + 1) *
                          m_this->Pole(i, j + 1).Coord() -
                      m_this->Weight(i, j) * m_this->Pole(i, j).Coord());
      wu.emplace_back(theta *
                      fabs(m_this->Weight(i, j + 1) - m_this->Weight(i, j)));
  ...
  for (auto i = ulow; i < uhigh; i++) {
    for (auto j = vlow; j < vhigh; j++) {
      pv.emplace_back(m_this->Weight(i + 1, j) *
                          m_this->Pole(i + 1, j).Coord() -
                      m_this->Weight(i, j) * m_this->Pole(i, j).Coord());
      wv.emplace_back(theta *
                      fabs(m_this->Weight(i + 1, j) - m_this->Weight(i, j)));
  ...
  for (int i = 0; i < pu.size() - 1; i++) {
    for (int j = 0; j < pv.size() - 1; j++) {
      double d = (fabs(pu[i] * pv[j]) + wu[i] * wv[j]) *
                 (m_this->UDegree() + 1) * (m_this->VDegree() + 1);
      if (d > F) {
        F = d;
      }
    }
...
std::vector<float>
OCCT_NURBS_SURFACE::get_bezierControlPoints(Standard_Integer span_u,
                                            Standard_Integer span_v) {
